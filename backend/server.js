...
try {
  ...
  console.warn("⚠️ Razorpay failed to initialize. Payments will not work.");
}
...
// ── AI Recovery Routes ───────────────────────────────────────
const aiRecoveryRoutes = require('./aiRecoveryRoutes');
app.use('/api/ai/recovery', aiRecoveryRoutes(db));

// ── AI Commerce Routes ───────────────────────────────────────
const llmService = require('./ai/llmService');
app.post('/api/ai/commerce/recommend', async (req, res) => {
  try {
    const { userIntent, pageContext, user, cart, chatHistory } = req.body;
    const recommendation = await llmService.generateProductRecommendation(userIntent, pageContext, user, cart, chatHistory);
    res.json({ success: true, recommendation });
  } catch (error) {
    console.error("AI Recommendation Error:", error.message);
    res.status(500).json({ success: false, recommendation: { text: "I'm sorry, I'm having trouble processing that. Please try again.", products: [], action: null } });
  }
});
...
app.get('/api/ai/health', async (req, res) => {
  try {
    ...
    const hasKey = !!process.env.AI_API_KEY && process.env.AI_API_KEY.trim() !== '' && process.env.AI_API_KEY !== '<real Gemini API key>';

    let testResponse = null;
    let isWorking = false;

    if (hasKey && llmService.genAI) {
      try {
        const result = await llmService.genAI.models.generateContent({
    ...
    res.json({
      ...
      hasKey,
      model: modelName || 'Not configured',
      isWorking,
      testResponse
    });
  } catch (error) {
...
(async () => {
  try {
    ...
    `);
    console.log('✅  payment_attempts table ready');
    ...
    `);
    console.log('✅  abandoned_carts table ready');

    await db.query(`
      CREATE TABLE IF NOT EXISTS ai_recovery_cases (
        id INT AUTO_INCREMENT PRIMARY KEY,
        type ENUM('payment_failure', 'abandoned_cart'),
        reference_id INT,
        user_id INT,
        status ENUM('pending', 'approved', 'rejected', 'recovered', 'failed') DEFAULT 'pending',
    ...
    `);
    console.log('✅  ai_recovery_cases table ready');
...
// ════════════════════════════════════════════════════════════════
//  RAZORPAY — Failure Hook for AI Recovery
// ════════════════════════════════════════════════════════════════
const aiAgent = require('./ai/agent');

app.post('/api/razorpay/failure', authMiddleware, async (req, res) => {
  try {
    const { order_id, error_code, error_description, amount } = req.body;
    
    // Log payment attempt failure
    const [result] = await db.query(`
      INSERT INTO payment_attempts 
      (user_id, cart_total, razorpay_order_id, status, error_code, error_description)
      VALUES (?, ?, ?, 'failed', ?, ?)
    `, [req.user.id, amount, order_id, error_code, error_description]);

    // Trigger AI Agent
    await aiAgent.processDropoffEvent(
      db, 
      'payment_failure', 
      result.insertId, 
      req.user.id, 
      { reason: error_description, cartTotal: amount }
    );

    res.json({ success: true });
  } catch (e) {
    console.error('Razorpay failure log error:', e);
